...
export default {
  ...
  computed: {
    ...
    rightIcon() {
      return this.iconRight || (this.isOpen && this.options.length > 0 ? 'angle-up' : 'angle-down');
    },
  ...
  watch: {
    selectionName(val) {
      if (this.options.length === 0 && val.length) {
  ...
  methods: {
    onOptionSelect(item) {
      if (!this.multiselect) {
        ...
        this.selectionName =
          this.selection.length > this.multiselectCount
            ? `${this.texts.chosen}: ${this.selection.length}`
            : options.join(', ');

        this.$emit(
          'update:model-value',
          this.selection.length > this.multiselectCount ? options : this.selectionName.split(', ')
        );
      }
    },
    ...
    onToggleSelect() {
      !this.isOpen ? this.onOpenSelect() : this.onCloseSelect();
    },
